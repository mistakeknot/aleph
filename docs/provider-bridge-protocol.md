...
classification, not a recovery hint: the bridge rejects the request with
`MISSING_EXECUTABLE` (-32004) instead of the generic `BRIDGE_ERROR`, throwing
`experimental_BridgeMissingExecutableError` or naming the code in `sendError`.
The host daemon reports that rejection as `missing_executable` without reading
the message, so the picker can say the CLI is missing whatever prose the bridge
chose. A bridge that keeps using `BRIDGE_ERROR` is classified generically, as
before.
